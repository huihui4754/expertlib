...
2.在收到用户的消息后，首先获取用户的dailog_id 和 user_id ,从map 中获取该id的聊天实例，如果没有就去初始化一个,获取用户的意图，实例化相应程序库nodejs实例，和每个用户dailog_id 聊天时时都要在独立的线程中
3.这个dailog_id 的用户后续聊天都会直接转给这个实例
4.用户的消息会直接发给程序库，程序库中会引导用户对话，当具体需求实现后需要主动停止聊天，回复 2002
5.当连接上程序库超过几个小时没有消息结束聊天，销毁 map 中 dailog_id 的对象，回复2002
6.提供http 接口，让nodejs 程序可以调用查询，保存相关数据
