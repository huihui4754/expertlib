...
import (
	...
	"github.com/huihui4754/expertlib/ssemcpclient"
...
func main() {
	...
	defer cancel()
	mcpclient, err := ssemcpclient.NewSSEFuncCall(ctx, "http://zhangshy.faas.ipanel.cn/9694f10bb3a34142fa062c575bc2b0c9/v1/sse")
	if err != nil {
		logger.Errorf("创建SSE函数调用客户端失败: %v", err)
		panic("mcp sse 链接失败")
	}

	chatx.SetModelContextFunctionTools(mcpclient.GetOriginalMcpTools())
