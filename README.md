...
expert ，chat ,program 三个模块可以程序中单独使用，也可以写在一个程序中绑定一起使用，完整例子请查看example 目录
...
-   **核心事件 (`event_type`)**:
    -   `1001` (客户端 -> 服务端): 用户或者多轮对话转发用户的消息。如果消息中包含 `intention` 字段，则会直接路由到指定工具；否则将进入意图识别流程。
    -   `1002` (客户端 -> 服务端): 用户主动终止当前对话。
    -   `2001` (服务端 -> 客户端): 程序库或 多轮对话 发送对话消息给用户。
    -   `2002` (服务端 -> 客户端): 程序库告知对话已结束，控制权交还给专家。
    -   `2003` (服务端 -> 客户端): 程序库表示无法处理当前请求，请求 专家 重新分配工具（肌肉）。
    -   `2004` (服务端 -> 客户端): 程序库表示没有找到该程序。